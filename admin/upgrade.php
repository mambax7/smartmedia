...
//require_once SMARTMEDIA_ROOT_PATH . "class/dbupdater.php";

$dbupdater = new Smartmedia\Dbupdater();
...
function checkTables()
{
    ...
    xoops_cp_header();
    //smartmedia_adminMenu(-1, _AM_SMARTMEDIA_DB_CHECKTABLES);
    //1. Determine previous release
    if (!smartmedia_TableExists('smartmedia_meta')) {
        $ver = '0.75';
    } else {
        if (!$ver = Utility::GetMeta('version')) {
    ...
    printf('<h2>' . _AM_SMARTMEDIA_DB_CURRENTVER . '</h2>', $currentVer);
    printf('<h2>' . _AM_SMARTMEDIA_DB_DBVER . '</h2>', $ver);

    if ($ver == $currentVer) {
        //No updates are necessary
        echo '<div>' . _AM_SMARTMEDIA_DB_NOUPDATE . '</div>';
    } elseif ($ver < $currentVer) {
        //Needs to upgrade
        echo '<div>' . _AM_SMARTMEDIA_DB_NEEDUPDATE . '</div>';
        echo '<form method="post" action="upgrade.php"><input type="hidden" name="op" value="upgradeDB"><input type="submit" value="' . _AM_SMARTMEDIA_DB_UPDATE_NOW . '"></form>';
    } else {
        //Tried to downgrade
        echo '<div>' . _AM_SMARTMEDIA_DB_NEEDINSTALL . '</div>';
    }
...
function upgradeDB()
{
    ...
    if (!smartmedia_TableExists('smartmedia_meta')) {
        ...
        if (!$ver = Utility::GetMeta('version')) {
            exit(_AM_SMARTMEDIA_DB_VERSION_ERR);
        }
    }
    ...
    xoops_cp_header();
    //smartmedia_adminMenu(-1, _AM_SMARTMEDIA_DB_UPDATE_DB);
    echo '<h2>' . _AM_SMARTMEDIA_DB_UPDATE_DB . '</h2>';
    $ret = true;
    ...
    switch ($ver) {
        case '0.75':
            set_time_limit(60);
            printf('<h3>' . _AM_SMARTMEDIA_DB_UPDATE_TO . '</h3>', '0.85');
            echo '<ul>';

            // Create table smartmedia_meta
            $table = new Smartmedia\Table('smartmedia_meta');
            $table->setStructure(
                "CREATE TABLE %s (
            ...
            $table = new Smartmedia\Table('smartmedia_clips');
            ...
            $table = new Smartmedia\Table('smartmedia_folders');
            $table->addAlteredField('counter', "INT( 11 ) DEFAULT '0' NOT NULL");
            $ret = $dbupdater->updateTable($table) && $ret;
            unset($table);

            //Drop table smartmedia_resolutions
            $table = new Smartmedia\Table('smartmedia_resolutions');
            $table->setFlagForDrop();
            $ret = $dbupdater->updateTable($table) && $ret;
            unset($table);

            // Drop table smartmedia_languages
            $table = new Smartmedia\Table('smartmedia_languages');
            $table->setFlagForDrop();
            $ret = $dbupdater->updateTable($table) && $ret;
            unset($table);

            // Drop table smartmedia_formats
            $table = new Smartmedia\Table('smartmedia_formats');
            $table->setFlagForDrop();
            $ret = $dbupdater->updateTable($table) && $ret;
            unset($table);
            ...
            $table->setData(
                '3, \'<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=6,0,0,0" height="{CLIP_HEIGHT}" width="{CLIP_WIDTH}">\r\n  <param name="movie" value="{CLIP_URL}">\r\n   <param name="quality" value="best">\r\n   <param name="play" value="{CLIP_AUTOPLAY}">\r\n   <embed \r\n     height="{CLIP_HEIGHT}" \r\n     pluginspage="http://www.macromedia.com/go/getflashplayer" \r\n      src="{CLIP_URL}" \r\n       type="application/x-shockwave-flash" \r\n       width="{CLIP_WIDTH}" \r\n       quality="best" \r\n     play="{CLIP_URL}">\r\n    </embed>\r\n</object>\', \'Flash\', \'swf\''
            );
            $table->setData(
                '4, \'<object id="MediaPlayer1" classid="CLSID:22d6f312-b0f6-11d0-94ab-0080c74c7e95" codebase="http://activex.microsoft.com/activex/controls/mplayer/en/nsmp2inf.cab# Version=5,1,52,701" standby="Loading Microsoft Windows� Media Player components..." type="application/x-oleobject" width="{CLIP_WIDTH}" height="{CLIP_HEIGHT}">\r\n <param name="fileName" value="{CLIP_URL}">\r\n    <param name="animationatStart" value="true">\r\n  <param name="transparentatStart" value="true">\r\n    <param name="autoStart" value="{CLIP_AUTOSTART}">\r\n <param name="showControls" value="true">\r\n  <param name="Volume" value="-300">\r\n    <embed \r\n     type="application/x-mplayer2" \r\n      pluginspage="http://www.microsoft.com/Windows/MediaPlayer/" \r\n        src="{CLIP_URL}" \r\n       name="MediaPlayer1" \r\n        width="{CLIP_WIDTH}" \r\n       height="{CLIP_HEIGHT}" \r\n     autostart="{CLIP_AUTOSTART}" \r\n       showcontrols="1" volume="-300">\r\n </embed>\r\n</object>\', \'Windows Media Player Audio\', \'wma\''
            );
            $table->setData(
                '5, \'<object id="RVOCX" classid="clsid:CFCDAA03-8BE4-11cf-B84B-0020AFBBCCFA" width="{CLIP_WIDTH}" height="{CLIP_HEIGHT}">\r\n  <param name="src" value="{CLIP_URL}">\r\n <param name="autostart" value="{CLIP_AUTOSTART}">\r\n <param name="controls" value="all">\r\n   <param name="console" value="audio">\r\n  <embed \r\n     type="audio/x-pn-realaudio-plugin" \r\n     src="{CLIP_URL}" \r\n       width="{CLIP_WIDTH}" \r\n       height="{CLIP_HEIGHT}" \r\n     autostart="{CLIP_AUTOSTART}" \r\n       controls="all" \r\n     console="audio">\r\n    </embed>\r\n</object>\', \'Real Player Audio\', \'rm\''
            );
            $table->setData(
                '6, \'<object id="RVOCX" classid="clsid:CFCDAA03-8BE4-11cf-B84B-0020AFBBCCFA" width="{CLIP_WIDTH}" height="{CLIP_HEIGHT}">\r\n  <param name="src" value="{CLIP_URL}">\r\n <param name="autostart" value="{CLIP_AUTOSTART}">\r\n <param name="controls" value="imagewindow,all">\r\n   <param name="console" value="video">\r\n  <embed \r\n     type="audio/x-pn-realaudio-plugin" \r\n     src="{CLIP_URL}" \r\n       width="{CLIP_WIDTH}" \r\n       height="{CLIP_HEIGHT}" \r\n     autostart="{CLIP_AUTOSTART}" \r\n       controls="imagewindow,all" \r\n     console="video">\r\n    </embed>\r\n</object>\', \'Real Player Video\', \'rm\''
            );
            $table->setData(
                '7, \'<object classid="clsid:02BF25D5-8C17-4B23-BC80-D3488ABDDC6B" width="{CLIP_WIDTH}" height="{CLIP_HEIGHT}"  codebase="http://www.apple.com/qtactivex/qtplugin.cab#version=6,0,2,0">\\\r\n   <param name="type" value="{CLIP_URL}">\r\n    <param name="autoplay" value="{CLIP_AUTOPLAY}">\r\n   <param name="target" value="myself">\r\n  <param name="src" value="{CLIP_URL}">\r\n <param name="href" value="{CLIP_URL}">\r\n    <param name="pluginspage" value="http://www.apple.com/quicktime/download/indext.html">\r\n    <param name="ShowControls" value="1">\r\n <param name="ShowStatusBar" value="1">\r\n    <param name="showdisplay" value="0">\r\n  <embed \r\n     width="{CLIP_WIDTH}" \r\n       height="{CLIP_HEIGHT}" \r\n     src="{CLIP_URL}" \r\n       href="{CLIP_URL}" \r\n      type="video/quicktime" \r\n     target="myself" \r\n        border="0" \r\n     showcontrols="1" \r\n       showdisplay="0" \r\n        showstatusbar="1" \r\n      autoplay="{CLIP_AUTOPLAY}" \r\n     pluginspage="http://www.apple.com/quicktime/download/indext.html">\r\n  </embed>\r\n</object>\', \'QuickTime Audio\', \'mov\''
            );
            $ret = $dbupdater->updateTable($table) && $ret;
            unset($table);
            ...
            //set good format
            $clipHandler = Helper::getInstance()->getHandler('Clip');
            $clipHandler->updateAll('formatid', 2);
            echo '</ul>';
    }
    ...
    if ($ret) {
        printf(_AM_SMARTMEDIA_DB_UPDATE_OK, $newversion);
        $ret = Utility::SetMeta('version', $newversion);
    } else {
        printf(_AM_SMARTMEDIA_DB_UPDATE_ERR, $newversion);
    }
